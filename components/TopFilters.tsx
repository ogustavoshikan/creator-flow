...
export const TopFilters: React.FC<TopFiltersProps> = ({
  ...
  const getFilterBtnClass = (isActive: boolean) =>
    `px-4 py-1.5 rounded-full text-xs whitespace-nowrap transition-all flex items-center gap-1.5 ${isActive
  ...
  return (
    <header className={`w-full border-b border-border flex flex-col shrink-0 z-10 bg-background/95 backdrop-blur-md transition-all duration-slow ease-in-out ${isZenMode ? 'max-h-0 overflow-hidden opacity-0 py-0 border-b-0 blur-sm' : 'max-h-[300px] opacity-100 py-6 px-8 gap-6'}`}>
