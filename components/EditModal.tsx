...
export const EditModal: React.FC<EditModalProps> = ({ isOpen, onClose, task, onSave, onDelete }) => {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-background/80 backdrop-blur-sm transition-all duration-300 ${isClosing ? 'animate-fade-out opacity-0' : 'animate-fade-in opacity-100'}`}
      onClick={handleClose}
    >
      <div
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="relative">
                <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 z-10">
                  <span className={`h-2.5 w-2.5 rounded-full ${formData.status === 'Ideias' ? 'bg-yellow-500' :
